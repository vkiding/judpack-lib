...
function getPlatformApi(platform, platformRootDir) {
    ...
    if (!platformRootDir) {
        ...
        throw new Error('Current location is not a judpack project');
    }
  ...
  //WEEK_HOOK
    ...
    //         events.emit('warn', ' Using this version of judpack with older version of judpack-' + platform +
    //             ' is being deprecated. Consider upgrading to judpack-' + platform + '@' +
    //             platforms[platform].apiCompatibleSince + ' or newer.');
    //     } else {
    //         events.emit('warn', 'Error loading judpack-'+platform);
    //     }
    //
